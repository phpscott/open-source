...
// https://github.com/lutian/phpEditSubtitles
...
class phpEditSubtitles {
	...
	*/
	public function saveFile($newFile = '') {
		...
		else: $newFile = $newFile.'.'.$this->getType(); endif;
                $this->removeThisFile ($newFile);
                $subtitles = $this->getSubtitles();
                //print_r($subtitles); exit;
                $this->writeThisData ($this->dumpSubtitles(),$newFile);
		//file_put_contents($newFile,$this->dumpSubtitles());
	}
        
        public function outputVTT ()
        {
            return $this->dumpSubtitles();
        }
        
        public function writeThisData ($data,$newFile)
        {
        ...
        public function removeThisFile ($file)
        {
            $output=null; $retval=null;
            if (is_file($file))
            {
                echo "removeThisFile: Is FILE\n";
                exec('rm -f '.$file, $output, $retval);
            }
        }
	...
	public function readFile() 
        {
            ...
            foreach ($lines as $line_num => $data) 
            {
                ...
                $arr = $this->findSubtitleBlock($data);
                //print_r($arr); exit;
                $found = $arr['found'];
                ...
                else 
                {
                    if (false !== $started) 
                    {
                        //echo (trim($data) === "") ? "Trimmed: ".trim($data)."\n" : "Not Trimmed: ".trim($data)."\n";
                        if (trim($data) !== "")
                        {
                                $arrSubtitle[$i]['subtitle'] .= $data;
                        } 
                        else 
                        {
                                $started = false;
